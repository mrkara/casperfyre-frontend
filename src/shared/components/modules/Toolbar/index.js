  ...
  return (
    <div className='flex justify-between w-full pb-7'>
      <div className='flex items-center gap-x-1'>
        ...
        <select className='outline outline-1 px-1 font-semibold text-[10px]' value={params.limit} onChange={(e) => handleChange('limit', +e.target.value)}>
          {ENTRIES_PICKER.map(x => <option value={x}>{x}</option>)}
        </select>
        <p className='font-semibold text-[10px]'>entries</p>
      </div>
      <div>
        <Input className='bg-white w-40 py-0 px-1 text-[10px]' placeholder='Search' onChange={(e) => setSearchTerm(e.target.value)} />
      </div>
    </div>
  );
};
